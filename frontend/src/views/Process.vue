<template>
  <div class="process-page">
    ...
    <div class="main-content">
      <!-- Левая панель: отображение графа в реальном времени -->
          ...
          <div class="process-phase" :class="{ 'active': currentPhase === 1, 'completed': currentPhase > 1 }">
            ...
            <div class="phase-detail">
              <div class="detail-section">
                ...
                <div class="detail-content">
                  На основе сгенерированной онтологии документ разбивается на фрагменты, после чего через Zep API строится граф знаний с извлечением сущностей и связей
                </div>
              </div>
              ...
              <div class="detail-section" v-if="graphData">
                ...
                <div class="build-result">
                  ...
                  <div class="result-item">
                    ...
                    <span class="result-label">Рёбра связей</span>
                  </div>
                  <div class="result-item">
...
// Текущий ID проекта (может измениться с 'new' на фактический ID)
...
// Инициализация — обработка нового проекта или загрузка существующего
const initProject = async () => {
  const paramProjectId = route.params.projectId

  if (paramProjectId === 'new') {
    // Новый проект: получаем данные для загрузки из store
...
const handleNewProject = async () => {
  ...
  if (!pending.isPending || pending.files.length === 0) {
    error.value = 'Нет файлов для загрузки, вернитесь на главную страницу и попробуйте снова'
    loading.value = false
    return
  }
  ...
  try {
    ...
    if (response.success) {
      // Очистка данных ожидающей загрузки
      clearPendingUpload()

      // Обновление ID и данных проекта
...
const loadProject = async () => {
  try {
    ...
    if (response.success) {
      ...
      // Продолжение опроса для задачи в процессе построения
...
const fetchGraphData = async () => {
  try {
    // Сначала получаем информацию о проекте для получения graph_id
    ...
    if (projectResponse.success && projectResponse.data.graph_id) {
      ...
      if (graphResponse.success && graphResponse.data) {
        ...
        // Обновление рендеринга при изменении данных
  ...
  try {
    ...
    if (response.success) {
      ...
      if (task.status === 'completed') {
        ...
        // Обновление отображения прогресса до состояния завершения
        buildProgress.value = {
          progress: 100,
          message: 'Построение завершено, загрузка графа...'
        }

        // Повторная загрузка данных проекта для получения graph_id
        const projectResponse = await getProject(currentProjectId.value)
...
const renderGraph = () => {
  ...
  // Создание маппинга узлов для поиска имён
  const nodeMap = {}
  nodesData.forEach(n => {
    nodeMap[n.uuid] = n
  })
  ...
  const nodes = nodesData.map(n => ({
    ...
    rawData: n // сохраняем исходные данные
  }))

  // Создание множества ID узлов для фильтрации валидных рёбер
  ...
  // Отрисовка рёбер (включая прозрачную широкую линию для кликов)
  ...
  // Прозрачная широкая линия для области клика
  ...
  // Клик по пустому месту закрывает панель деталей
  svg.on('click', () => {
    closeDetailPanel()
  })

  simulation.on('tick', () => {
    // Обновление позиций всех рёбер (включая видимую и прозрачную линии)
    ...
    // Обновление позиций меток рёбер
    linkLabel
...
// Наблюдение за изменениями данных графа
watch(graphData, () => {
  if (graphData.value) {
...
/* Отображение связи в деталях рёбер */
.edge-relation {
  display: flex;
  align-items: center;
  flex-wrap: wrap;
