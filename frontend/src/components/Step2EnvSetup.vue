<template>
  <div class="env-setup-panel">
    <div class="scroll-container">
      ...
      <!-- Step 02: Генерация персонажей Agent -->
              ...
              <div class="platforms-grid">
                <div v-if="simulationConfig.twitter_config" class="platform-card">
                  <div class="platform-card-header">
                    <span class="platform-name">Платформа 1: Площадка / Лента</span>
                  </div>
      ...
      <!-- Step 04: Начальная оркестровка активации -->
    ...
    <div class="system-logs">
      <div class="log-header">
        <span class="log-title">ПАНЕЛЬ УПРАВЛЕНИЯ</span>
...
// State
const phase = ref(0) // 0: инициализация, 1: генерация персонажей, 2: генерация конфигурации, 3: завершение
...
// Дедупликация логов: запись последней выведенной ключевой информации
let lastLoggedMessage = ''
let lastLoggedProfileCount = 0
let lastLoggedConfigStage = ''

// Конфигурация количества раундов
const useCustomRounds = ref(false) // По умолчанию используются автоматически рассчитанные раунды
const customMaxRounds = ref(40)   // Рекомендуемое значение по умолчанию - 40 раундов
...
// Вычисление автоматически сгенерированного количества раундов из конфигурации (без жёстко заданных значений)
const autoGeneratedRounds = computed(() => {
  if (!simulationConfig.value?.time_config) {
    return null // Конфигурация ещё не сгенерирована
  }
  const totalHours = simulationConfig.value.time_config.total_simulation_hours
  const minutesPerRound = simulationConfig.value.time_config.minutes_per_round
  if (!totalHours || !minutesPerRound) {
    return null // Данные конфигурации неполные
  }
  const calculatedRounds = Math.floor((totalHours * 60) / minutesPerRound)
  // Минимум 40 (рекомендуемое значение), чтобы избежать аномалий ползунка
...
// Подсчёт общего количества связанных тем для всех персонажей
...
const handleStartSimulation = () => {
  // Формирование параметров для передачи родительскому компоненту
  const params = {}

  if (useCustomRounds.value) {
    // Пользовательское количество раундов
    params.maxRounds = customMaxRounds.value
    addLog(`Запуск симуляции, пользовательское кол-во раундов: ${customMaxRounds.value}`)
  } else {
    // Автоматически рассчитанное количество раундов
    addLog(`Запуск симуляции, авто-конфигурация раундов: ${autoGeneratedRounds.value}`)
  }
  ...
  try {
    ...
    if (res.success && res.data) {
      if (res.data.already_prepared) {
        addLog('Обнаружена завершённая подготовка, используются готовые данные')
      ...
      // Установка ожидаемого количества Agent из ответа prepare
      if (res.data.expected_entities_count) {
        expectedTotal.value = res.data.expected_entities_count
        addLog(`Из графа Zep извлечено ${res.data.expected_entities_count} сущностей`)
        if (res.data.entity_types && res.data.entity_types.length > 0) {
          addLog(`  └─ Типы сущностей: ${res.data.entity_types.join(', ')}`)
        }
      }

      addLog('Начало опроса статуса подготовки...')
      // Запуск опроса прогресса
      startPolling()
      // Запуск получения профилей в реальном времени
...
const pollPrepareStatus = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Анализ информации об этапе и вывод детальных логов
      if (data.progress_detail) {
        currentStage.value = data.progress_detail.current_stage_name || ''

        // Вывод детальных логов прогресса (без дублирования)
        ...
        // Вывод лога сообщения (без дублирования)
...
const fetchProfilesRealtime = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Обновление только при наличии валидного значения от API
      ...
      // Вывод лога прогресса генерации профилей (только при изменении количества)
      ...
      if (currentCount > 0 && currentCount !== lastLoggedProfileCount) {
        ...
        if (expectedTotal.value && currentCount >= expectedTotal.value) {
          addLog(`✓ Все ${currentCount} персонажей Agent сгенерированы`)
        }
      }
    }
  } catch (err) {
    console.warn('Ошибка получения профилей:', err)
  }
}
...
const fetchConfigRealtime = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Вывод логов этапа генерации конфигурации (без дублирования)
      if (data.generation_stage && data.generation_stage !== lastLoggedConfigStage) {
        lastLoggedConfigStage = data.generation_stage
        if (data.generation_stage === 'generating_profiles') {
          addLog('Генерация конфигурации персонажей Agent...')
        } else if (data.generation_stage === 'generating_config') {
          addLog('Вызов LLM для генерации параметров конфигурации симуляции...')
        }
      }
      ...
      if (data.config_generated && data.config) {
        ...
        // Вывод детальной сводки конфигурации
        if (data.summary) {
          addLog(`  ├─ Количество Agent: ${data.summary.total_agents} шт.`)
          addLog(`  ├─ Длительность: ${data.summary.simulation_hours} часов`)
          addLog(`  ├─ Начальных постов: ${data.summary.initial_posts_count} шт.`)
          addLog(`  ├─ Горячих тем: ${data.summary.hot_topics_count} шт.`)
        ...
        if (data.config.time_config) {
          ...
          addLog(`Конфигурация времени: ${tc.minutes_per_round} мин/раунд, всего ${Math.floor((tc.total_simulation_hours * 60) / tc.minutes_per_round)} раундов`)
        }
    ...
    console.warn('Ошибка получения конфигурации:', err)
  }
}

const loadPreparedData = async () => {
  phase.value = 2
  addLog('Загрузка существующих данных конфигурации...')

  // Финальное получение профилей
  await fetchProfilesRealtime()
  addLog(`Загружено ${profiles.value.length} персонажей Agent`)

  // Получение конфигурации (через realtime-интерфейс)
  try {
    const res = await getSimulationConfigRealtime(props.simulationId)
    if (res.success && res.data) {
      if (res.data.config_generated && res.data.config) {
        ...
        if (res.data.summary) {
          ...
          addLog(`  ├─ Длительность: ${res.data.summary.simulation_hours} часов`)
        ...
        // Конфигурация ещё не сгенерирована, запуск опроса
        addLog('Конфигурация генерируется, ожидание...')
        startConfigPolling()
      }
    }
  } catch (err) {
