    ...
    <div class="main-split-layout">
      ...
      <div class="left-panel report-style" ref="leftPanel">
        ...
        <div v-if="!reportOutline" class="waiting-placeholder">
          ...
          <span class="waiting-text">Ожидание агента отчёта...</span>
        </div>
      </div>
          ...
          <div v-if="chatTarget === 'report_agent'" class="report-agent-tools-card">
            <div class="tools-card-header">
              ...
              <div class="tools-card-info">
                <div class="tools-card-name">Агент отчёта — Чат</div>
                <div class="tools-card-subtitle">Диалоговый агент с доступом к 4 инструментам и полной памятью MirPrognoz</div>
              </div>
              ...
              <div class="message-content">
                <div class="message-header">
                  <span class="sender-name">
                    {{ msg.role === 'user' ? 'Вы' : (chatTarget === 'report_agent' ? 'Агент отчёта' : (selectedAgent?.username || 'Агент')) }}
                  </span>
...
const chatHistoryCache = ref({}) // Кэш всех диалогов: { 'report_agent': [], 'agent_0': [], 'agent_1': [], ... }
...
const selectReportAgentChat = () => {
  ...
  // Восстановить диалог Report Agent
...
const selectAgent = (agent, idx) => {
  ...
  // Восстановить диалог этого агента
...
const renderMarkdown = (content) => {
  ...
  // Обёртка маркированных списков
  html = html.replace(/(<li class="md-li"[^>]*>.*?<\/li>\s*)+/g, '<ul class="md-ul">$&</ul>')
  // Обёртка нумерованных списков
  ...
  // Очистка <br> перед открывающим тегом параграфа после блочных элементов
  html = html.replace(/(<\/ol>|<\/ul>|<\/blockquote>)<br>(<p|<div)/g, '$1$2')

  // Исправление нумерации прерывистых списков: когда одиночные <ol> разделены абзацами, сохраняем нарастающую нумерацию
...
const sendToAgent = async (message) => {
  ...
  if (chatHistory.value.length > 1) {
    const historyContext = chatHistory.value
      ...
      .slice(-6)
      .map(msg => `${msg.role === 'user' ? 'Спрашивающий' : 'Вы'}：${msg.content}`)
      .join('\n')
  ...
  if (res.success && res.data) {
    // Правильный путь данных: res.data.result.results — это словарь объектов
    // Формат: {"twitter_0": {...}, "reddit_0": {...}} или одна платформа {"reddit_0": {...}}
    const resultData = res.data.result || res.data
    const resultsDict = resultData.results || resultData
    
    // Преобразование словаря объектов в массив, приоритет ответов с платформы reddit
...
const submitSurvey = async () => {
  ...
  addLog(`Отправка анкеты ${selectedAgents.value.size} объектам...`)
...
/* Left Panel - Report Style (полностью идентично Step4Report.vue) */
.left-panel.report-style {
  width: 45%;
  min-width: 450px;
  background: #FFFFFF;
...
/* Исправление нумерации списков - CSS-счётчики для последовательной нумерации нескольких ol */
.message-text {
