              ...
              <div class="card-body">
                ...
                <template v-if="action.action_type === 'REPOST'">
                  <div class="repost-info">
                    ...
                    <span class="repost-label">Репост от @{{ action.action_args?.original_author_name || 'User' }}</span>
                  </div>
                ...
                <template v-if="action.action_type === 'LIKE_POST'">
                  <div class="like-info">
                    ...
                    <span class="like-label">Оценил пост @{{ action.action_args?.post_author_name || 'User' }}</span>
                  </div>
                ...
                <!-- CREATE_COMMENT: Публикация комментария -->
                ...
                <template v-if="action.action_type === 'FOLLOW'">
                  <div class="follow-info">
                    ...
                    <span class="follow-label">Подписался на @{{ action.action_args?.target_user || action.action_args?.user_id || 'User' }}</span>
                  </div>
                </template>
                ...
                <!-- DO_NOTHING: Бездействие (пропуск) -->
                ...
                <!-- Универсальный fallback: неизвестный тип или контент, не обработанный выше -->
...
// Подсчёт действий по платформам
...
// Форматирование прошедшего времени симуляции (на основе раундов и минут на раунд)
...
// Прошедшее время симуляции для платформы Twitter
const twitterElapsedTime = computed(() => {
  return formatElapsedTime(runStatus.value.twitter_current_round || 0)
})

// Прошедшее время симуляции для платформы Reddit
...
const doStartSimulation = async () => {
  ...
  // Сброс всех состояний для исключения влияния предыдущей симуляции
  resetAllState()
  ...
  try {
    const params = {
      ...
      enable_graph_memory_update: true  // Включение динамического обновления графа
    }

    if (props.maxRounds) {
      params.max_rounds = props.maxRounds
      addLog(`Установлено максимальное количество раундов: ${props.maxRounds}`)
    }

    addLog('Включён режим динамического обновления графа')

    const res = await startSimulation(params)

    if (res.success && res.data) {
      if (res.data.force_restarted) {
        addLog('✓ Старые логи симуляции очищены, симуляция перезапущена')
      }
...
// Отслеживание предыдущего раунда каждой платформы для обнаружения изменений
...
const fetchRunStatus = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Обнаружение изменения раунда каждой платформы и вывод лога
      ...
      // Дополнительная проверка: если бэкенд ещё не обновил runner_status,
      // но платформы уже сообщили о завершении
      const platformsCompleted = checkPlatformsCompleted(data)

      if (isCompleted || platformsCompleted) {
        if (platformsCompleted && !isCompleted) {
          addLog('✓ Обнаружено завершение всех платформ')
        }
...
// Проверка завершения всех активных платформ
const checkPlatformsCompleted = (data) => {
  // Если нет данных о платформах, возвращаем false
  if (!data) return false

  // Проверка статуса завершения каждой платформы
  const twitterCompleted = data.twitter_completed === true
  const redditCompleted = data.reddit_completed === true

  // Определение, какие платформы были активны
  const twitterEnabled = (data.twitter_actions_count > 0) || data.twitter_running || twitterCompleted
  const redditEnabled = (data.reddit_actions_count > 0) || data.reddit_running || redditCompleted

  // Если ни одна платформа не активна, возвращаем false
...
const fetchRunStatusDetail = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Инкрементальное добавление новых действий (с дедупликацией)
      ...
      // Автоматическая прокрутка не выполняется, пользователь может свободно просматривать временную шкалу
