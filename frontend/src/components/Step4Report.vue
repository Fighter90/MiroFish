                ...
                <div class="timeline-body" :class="{ 'collapsed': isLogCollapsed(log) }" @click="toggleLogExpand(log)">
                  ...
                  <template v-if="log.action === 'llm_response'">
                    ...
                    <div v-if="log.details?.has_final_answer" class="final-answer-hint">
                      ...
                      <span>Раздел «{{ log.section_title }}» сгенерирован</span>
                    </div>
                ...
                <div class="timeline-footer" v-if="log.elapsed_seconds || (log.action === 'tool_call' && log.details?.parameters) || log.action === 'tool_result' || (log.action === 'llm_response' && log.details?.response)">
                  ...
                  <div class="footer-actions">
                    ...
                    <button v-if="log.action === 'tool_result'" class="action-btn" @click.stop="toggleRawResult(log.timestamp, $event)">
                      {{ showRawResult[log.timestamp] ? 'Структурированный вид' : 'Исходный вывод' }}
                    </button>
...
const toggleRawResult = (timestamp, event) => {
  // Сохранить позицию кнопки относительно вьюпорта
  ...
  // После обновления DOM скорректировать прокрутку, чтобы кнопка осталась на месте
  if (button && buttonTopBeforeToggle !== undefined && rightPanel.value) {
    nextTick(() => {
      ...
      // Скорректировать позицию прокрутки
...
const toolConfig = {
  'insight_forge': {
    ...
    color: 'purple',
    icon: 'lightbulb' // Иконка лампочки - символ инсайта
  },
  'panorama_search': {
    name: 'Полный поиск',
    color: 'blue',
    icon: 'globe' // Иконка глобуса - символ панорамного поиска
  },
  'interview_agents': {
    name: 'Интервью агентов',
    color: 'green',
    icon: 'users' // Иконка пользователей - символ диалога
  },
  'quick_search': {
    name: 'Быстрый поиск',
    color: 'orange',
    icon: 'zap' // Иконка молнии - символ быстроты
  },
  'get_graph_statistics': {
    name: 'Статистика графа',
    color: 'cyan',
    icon: 'chart' // Иконка графика - символ статистики
  },
  'get_entities_by_type': {
    name: 'Entity Query',
    color: 'pink',
    icon: 'database' // Иконка базы данных - символ сущностей
  }
}
...
const parseInsightForge = (text) => {
  ...
  try {
    // Извлечь вопрос анализа
    const queryMatch = text.match(/(?:Анализируемый вопрос|分析问题):\s*(.+?)(?:\n|$)/)
    if (queryMatch) result.query = queryMatch[1].trim()

    // Извлечь сценарий прогнозирования
    const reqMatch = text.match(/(?:Сценарий прогноза|预测场景):\s*(.+?)(?:\n|$)/)
    if (reqMatch) result.simulationRequirement = reqMatch[1].trim()

    // Извлечь статистику
    const factMatch = text.match(/(?:Релевантные прогнозные факты|相关预测事实):\s*(\d+)/)
    const entityMatch = text.match(/(?:Вовлечённые сущности|涉及实体):\s*(\d+)/)
    const relMatch = text.match(/(?:Цепочки связей|关系链):\s*(\d+)/)
    if (factMatch) result.stats.facts = parseInt(factMatch[1])
    if (entityMatch) result.stats.entities = parseInt(entityMatch[1])
    if (relMatch) result.stats.relationships = parseInt(relMatch[1])

    // Извлечь подвопросы - полное извлечение без ограничения
    const subQSection = text.match(/### (?:Анализируемые подвопросы|分析的子问题)\n([\s\S]*?)(?=\n###|$)/)
    if (subQSection) {
      const lines = subQSection[1].split('\n').filter(l => l.match(/^\d+\./))
      result.subQueries = lines.map(l => l.replace(/^\d+\.\s*/, '').trim()).filter(Boolean)
    }
    
    // Извлечь ключевые факты - полное извлечение без ограничения
    const factsSection = text.match(/### (?:\[Ключевые факты\]|【关键事实】)[\s\S]*?\n([\s\S]*?)(?=\n###|$)/)
    if (factsSection) {
    ...
    // Извлечь основные сущности - полное извлечение, включая аннотации и число связанных фактов
    const entitySection = text.match(/### (?:\[Ключевые сущности\]|【核心实体】)\n([\s\S]*?)(?=\n###|$)/)
    if (entitySection) {
      const entityText = entitySection[1]
      // Разделить блоки сущностей по "- **"
      const entityBlocks = entityText.split(/\n(?=- \*\*)/).filter(b => b.trim().startsWith('- **'))
      result.entities = entityBlocks.map(block => {
        const nameMatch = block.match(/^-\s*\*\*(.+?)\*\*\s*\((.+?)\)/)
        const summaryMatch = block.match(/(?:Описание|摘要):\s*"?(.+?)"?(?:\n|$)/)
        const relatedMatch = block.match(/(?:Связанные факты|相关事实):\s*(\d+)/)
        return {
    ...
    // Извлечь цепочки связей - полное извлечение без ограничения
    const relSection = text.match(/### (?:\[Цепочки связей\]|【关系链】)\n([\s\S]*?)(?=\n###|$)/)
    if (relSection) {
...
const parsePanorama = (text) => {
  ...
  try {
    // Извлечь запрос
    const queryMatch = text.match(/(?:Запрос|查询):\s*(.+?)(?:\n|$)/)
    if (queryMatch) result.query = queryMatch[1].trim()

    // Извлечь статистику
    const nodesMatch = text.match(/(?:Всего узлов|总节点数):\s*(\d+)/)
    const edgesMatch = text.match(/(?:Всего рёбер|总边数):\s*(\d+)/)
    const activeMatch = text.match(/(?:Текущие актуальные факты|当前有效事实):\s*(\d+)/)
    const histMatch = text.match(/(?:Исторические\/устаревшие факты|历史\/过期事实):\s*(\d+)/)
    ...
    // Извлечь текущие актуальные факты - полное извлечение без ограничения
    const activeSection = text.match(/### (?:\[Текущие актуальные факты\]|【当前有效事实】)[\s\S]*?\n([\s\S]*?)(?=\n###|$)/)
    if (activeSection) {
      const lines = activeSection[1].split('\n').filter(l => l.match(/^\d+\./))
      result.activeFacts = lines.map(l => {
        // Удалить нумерацию и кавычки
        const factText = l.replace(/^\d+\.\s*/, '').replace(/^"|"$/g, '').trim()
        return factText
      }).filter(Boolean)
    }
    
    // Извлечь исторические/устаревшие факты - полное извлечение без ограничения
    const histSection = text.match(/### (?:\[Исторические\/устаревшие факты\]|【历史\/过期事实】)[\s\S]*?\n([\s\S]*?)(?=\n###|$)/)
    if (histSection) {
    ...
    // Извлечь связанные сущности - полное извлечение без ограничения
    const entitySection = text.match(/### (?:\[Задействованные сущности\]|【涉及实体】)\n([\s\S]*?)(?=\n###|$)/)
    if (entitySection) {
...
const parseInterview = (text) => {
  ...
  try {
    // Извлечь тему интервью
    const topicMatch = text.match(/\*\*(?:Тема интервью|采访主题):\*\*\s*(.+?)(?:\n|$)/)
    if (topicMatch) result.topic = topicMatch[1].trim()

    // Извлечь число опрошенных (напр. "5 / 9 агентов симуляции")
    const countMatch = text.match(/\*\*(?:Количество интервью|采访人数):\*\*\s*(\d+)\s*\/\s*(\d+)/)
    if (countMatch) {
    ...
    // Извлечь обоснование выбора респондентов
    const reasonMatch = text.match(/### (?:Обоснование выбора респондентов|采访对象选择理由)\n([\s\S]*?)(?=\n---\n|\n### (?:Протокол интервью|采访实录))/)
    if (reasonMatch) {
      result.selectionReason = reasonMatch[1].trim()
    }
    
    // Разобрать обоснование выбора каждого респондента
    const parseIndividualReasons = (reasonText) => {
      ...
      for (const line of lines) {
        ...
        // Пример: 1. **выпускник_345 (index=1)**: как выпускник...
        ...
        // Формат 2: - выбрать имя (index X): причина
        // Пример: - выбрать родитель_601 (index 0): как представитель родителей...
        if (!headerMatch) {
          headerMatch = line.match(/^-\s*(?:выбрать|选择)([^（(]+)(?:[（(]index\s*=?\s*\d+[)）])?[：:]\s*(.*)/)
          if (headerMatch) {
            name = headerMatch[1].trim()
            reasonStart = headerMatch[2]
          }
        ...
        // Пример: - **родитель_601 (index 0)**: как представитель родителей...
        if (!headerMatch) {
        ...
        if (name) {
          // Сохранить причину предыдущего респондента
          if (currentName && currentReason.length > 0) {
            reasons[currentName] = currentReason.join(' ').trim()
          }
          // Начать нового респондента
          currentName = name
          currentReason = reasonStart ? [reasonStart.trim()] : []
        } else if (currentName && line.trim() && !line.match(/^(?:не выбран|未选)|^(?:в итоге|综上)|^(?:итоговый выбор|最终选择)/)) {
          // Продолжение обоснования (исключая заключительные абзацы)
          currentReason.push(line.trim())
        }
      }
      
      // Сохранить причину последнего респондента
      if (currentName && currentReason.length > 0) {
        reasons[currentName] = currentReason.join(' ').trim()
      }
    ...
    // Извлечь каждую запись интервью
    const interviewBlocks = text.split(/#### (?:Интервью|采访) #\d+:/).slice(1)
    ...
    interviewBlocks.forEach((block, index) => {
      ...
      // Извлечь заголовок (напр. "студент", "педагог" и т.д.)
      ...
      if (nameRoleMatch) {
        ...
        // Установить причину выбора этого респондента
        interview.selectionReason = individualReasons[interview.name] || ''
      }
      
      // Извлечь краткую биографию
      const bioMatch = block.match(/_(?:Описание|简介):\s*([\s\S]*?)_\n/)
      if (bioMatch) {
        interview.bio = bioMatch[1].trim().replace(/\.\.\.$/, '...')
      }
      
      // Извлечь список вопросов
      const qMatch = block.match(/\*\*Q:\*\*\s*([\s\S]*?)(?=\n\n\*\*A:\*\*|\*\*A:\*\*)/)
      if (qMatch) {
        const qText = qMatch[1].trim()
        // Разделить вопросы по номерам
        const questions = qText.split(/\n\d+\.\s+/).filter(q => q.trim())
        if (questions.length > 0) {
          // Если первый вопрос начинается с "1.", требуется специальная обработка
      ...
      // Извлечь ответы - разделить по Twitter и Reddit
      const answerMatch = block.match(/\*\*A:\*\*\s*([\s\S]*?)(?=\*\*(?:Ключевые цитаты|关键引言)|$)/)
      if (answerMatch) {
        const answerText = answerMatch[1].trim()
        
        // Разделить ответы Twitter и Reddit
        const twitterMatch = answerText.match(/(?:\[Ответ на платформе Twitter\]|【Twitter平台回答】)\n?([\s\S]*?)(?=(?:\[Ответ на платформе Reddit\]|【Reddit平台回答】)|$)/)
        const redditMatch = answerText.match(/(?:\[Ответ на платформе Reddit\]|【Reddit平台回答】)\n?([\s\S]*?)$/)
        ...
        // Логика отката платформы (совместимость со старым форматом: только одна платформа)
        if (!twitterMatch && redditMatch) {
          // Только Reddit-ответ, копировать как дефолтный только если не заглушка
          if (interview.redditAnswer && interview.redditAnswer !== '(на этой платформе ответ не получен)' && interview.redditAnswer !== '（该平台未获得回复）') {
            interview.twitterAnswer = interview.redditAnswer
          }
        } else if (twitterMatch && !redditMatch) {
          if (interview.twitterAnswer && interview.twitterAnswer !== '(на этой платформе ответ не получен)' && interview.twitterAnswer !== '（该平台未获得回复）') {
            interview.redditAnswer = interview.twitterAnswer
          }
        } else if (!twitterMatch && !redditMatch) {
          // Нет меток платформ (очень старый формат), весь текст как ответ
          interview.twitterAnswer = answerText
        }
      }
      
      // Извлечь ключевые цитаты (совместимость с разными форматами кавычек)
      const quotesMatch = block.match(/\*\*(?:Ключевые цитаты|关键引言):\*\*\n([\s\S]*?)(?=\n---|\n####|$)/)
      if (quotesMatch) {
        const quotesText = quotesMatch[1]
        // Приоритетное совпадение формата > "text"
        let quoteMatches = quotesText.match(/> "([^"]+)"/g)
        // Откат: совпадение > "text" или > \u201Ctext\u201D (кит. кавычки)
        if (!quoteMatches) {
    ...
    // Извлечь сводку интервью
    const summaryMatch = text.match(/### (?:Сводка интервью и ключевые выводы|采访摘要与核心观点)\n([\s\S]*?)$/)
    if (summaryMatch) {
...
const parseQuickSearch = (text) => {
  ...
  try {
    ...
    const queryMatch = text.match(/(?:Поисковый запрос|搜索查询):\s*(.+?)(?:\n|$)/)
    if (queryMatch) result.query = queryMatch[1].trim()

    // Извлечь количество результатов
    const countMatch = text.match(/(?:Найдено\s*(\d+)\s*релевантных|找到\s*(\d+)\s*条)/)
    if (countMatch) result.count = parseInt(countMatch[1] || countMatch[2])

    // Извлечь связанные факты - полное извлечение без ограничения
    const factsSection = text.match(/### (?:Релевантные факты|相关事实):\n([\s\S]*)$/)
    if (factsSection) {
      const lines = factsSection[1].split('\n').filter(l => l.match(/^\d+\./))
      result.facts = lines.map(l => l.replace(/^\d+\.\s*/, '').trim()).filter(Boolean)
    }
    
    // Попытка извлечь информацию о рёбрах (при наличии)
    const edgesSection = text.match(/### (?:Релевантные рёбра|相关边):\n([\s\S]*?)(?=\n###|$)/)
    if (edgesSection) {
    ...
    // Попытка извлечь информацию об узлах (при наличии)
    const nodesSection = text.match(/### (?:Релевантные узлы|相关节点):\n([\s\S]*?)(?=\n###|$)/)
    if (nodesSection) {
    ...
    return () => h('div', { class: 'insight-display' }, [
      ...
      h('div', { class: 'insight-tabs' }, [
        h('button', {
          ...
          h('span', { class: 'tab-label' }, `Ключевые воспоминания (${props.result.facts.length})`)
        ]),
        h('button', {
          class: ['insight-tab', { active: activeTab.value === 'entities' }],
          onClick: () => { activeTab.value = 'entities' }
        }, [
          h('span', { class: 'tab-label' }, `Основные сущности (${props.result.entities.length})`)
        ]),
        h('button', {
      ...
      h('div', { class: 'insight-content' }, [
        ...
        activeTab.value === 'facts' && props.result.facts.length > 0 && h('div', { class: 'facts-panel' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'Последние ключевые факты из хронологической памяти'),
            h('span', { class: 'panel-count' }, `Всего ${props.result.facts.length} зап.`)
          ]),
        ...
        activeTab.value === 'entities' && props.result.entities.length > 0 && h('div', { class: 'entities-panel' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'Основные сущности'),
          ...
          h('div', { class: 'entities-grid' },
            (expandedEntities.value ? props.result.entities : props.result.entities.slice(0, 12)).map((entity, i) => 
              h('div', { class: 'entity-tag', key: i, title: entity.summary || '' }, [
                ...
                entity.relatedFactsCount > 0 && h('span', { class: 'entity-fact-count' }, `${entity.relatedFactsCount} зап.`)
              ])
            )
          ),
        ...
        activeTab.value === 'relations' && props.result.relations.length > 0 && h('div', { class: 'relations-panel' }, [
          h('div', { class: 'panel-header' }, [
            ...
            h('span', { class: 'panel-count' }, `Всего ${props.result.relations.length} зап.`)
          ]),
        ...
        activeTab.value === 'subqueries' && props.result.subQueries.length > 0 && h('div', { class: 'subqueries-panel' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'Подвопросы дрейфового анализа'),
        ...
        // Empty state
        activeTab.value === 'facts' && props.result.facts.length === 0 && h('div', { class: 'empty-state' }, 'Нет ключевых воспоминаний'),
        activeTab.value === 'entities' && props.result.entities.length === 0 && h('div', { class: 'empty-state' }, 'Нет основных сущностей'),
  ...
  setup(props) {
    ...
    return () => h('div', { class: 'panorama-display' }, [
      ...
      h('div', { class: 'panorama-header' }, [
        h('div', { class: 'header-main' }, [
          h('div', { class: 'header-title' }, 'Полный поиск'),
          h('div', { class: 'header-stats' }, [
            h('span', { class: 'stat-item' }, [
      ...
      h('div', { class: 'panorama-tabs' }, [
        h('button', {
          ...
          h('span', { class: 'tab-label' }, `Актуальные воспоминания (${props.result.activeFacts.length})`)
        ]),
        h('button', {
      ...
      h('div', { class: 'panorama-content' }, [
        ...
        activeTab.value === 'active' && h('div', { class: 'facts-panel active-facts' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'Актуальные воспоминания'),
            h('span', { class: 'panel-count' }, `Всего ${props.result.activeFacts.length} зап.`)
          ]),
          ...
          ) : h('div', { class: 'empty-state' }, 'Нет актуальных воспоминаний'),
          props.result.activeFacts.length > INITIAL_SHOW_COUNT && h('button', {
            class: 'expand-btn',
            onClick: () => { expandedActive.value = !expandedActive.value }
          }, expandedActive.value ? `Свернуть ▲` : `Показать все ${props.result.activeFacts.length} зап. ▼`)
        ]),
        ...
        activeTab.value === 'historical' && h('div', { class: 'facts-panel historical-facts' }, [
          h('div', { class: 'panel-header' }, [
            ...
            h('span', { class: 'panel-count' }, `Всего ${props.result.historicalFacts.length} зап.`)
          ]),
            ...
            (expandedHistorical.value ? props.result.historicalFacts : props.result.historicalFacts.slice(0, INITIAL_SHOW_COUNT)).map((fact, i) => 
              h('div', { class: 'fact-item historical', key: i }, [
                ...
                h('div', { class: 'fact-content' }, [
                  // Попытка извлечь информацию о времени [time - time]
                  (() => {
          ...
          ) : h('div', { class: 'empty-state' }, 'Нет исторических воспоминаний'),
          props.result.historicalFacts.length > INITIAL_SHOW_COUNT && h('button', {
            class: 'expand-btn',
            onClick: () => { expandedHistorical.value = !expandedHistorical.value }
          }, expandedHistorical.value ? `Свернуть ▲` : `Показать все ${props.result.historicalFacts.length} зап. ▼`)
        ]),
...
const InterviewDisplay = {
  ...
  setup(props) {
    ...
    // Для каждой пары вопрос-ответ поддерживать независимое состояние выбора платформы
    const platformTabs = reactive({}) // { 'agentIdx-qIdx': 'twitter' | 'reddit' }
    
    // Получить текущий выбор платформы для данного вопроса
    const getPlatformTab = (agentIdx, qIdx) => {
      const key = `${agentIdx}-${qIdx}`
      return platformTabs[key] || 'twitter'
    }
    
    // Установить выбор платформы для данного вопроса
    ...
    // Проверить, является ли текст заглушкой платформы
    const isPlaceholderText = (text) => {
      if (!text) return true
      const t = text.trim()
      return t === '(на этой платформе ответ не получен)' || t === '（该平台未获得回复）' || t === '(该平台未获得回复)' || t === '[无回复]'
    }

    // Попытка разделить ответ по номерам вопросов
    const splitAnswerByQuestions = (answerText, questionCount) => {
      ...
      // Поддержка форматов нумерации:
      // 1. "Вопрос X:" (русский формат) или "问题X：" (китайский формат, совместимость)
      // 2. "1. " или "\n1. " (число+точка, совместимость со старым форматом)
      let matches = []
      let match

      // Приоритетная попытка формата "Вопрос X:" или "问题X："
      const cnPattern = /(?:^|[\r\n]+)(?:Вопрос|问题)[\s]*(\d+)[：:]\s*/g
      ...
      // Если не совпало, откат к формату "число."
      if (matches.length === 0) {
        const numPattern = /(?:^|[\r\n]+)(\d+)\.\s+/g
      ...
      // Если нумерация не найдена или найден только один номер, вернуть целиком
      if (matches.length <= 1) {
        const cleaned = answerText
          .replace(/^(?:Вопрос|问题)[\s]*\d+[：:]\s*/, '')
          .replace(/^\d+\.\s+/, '')
          .trim()
        return [cleaned || answerText]
      }

      // Извлечь части по номерам
      const parts = []
    ...
    // Получить ответ для данного вопроса
    const getAnswerForQuestion = (interview, qIdx, platform) => {
      ...
      // Разделение не удалось: для первого вопроса вернуть полный ответ, для остальных пусто
      return qIdx === 0 ? answer : ''
    }
    
    // Проверить, есть ли ответы с двух платформ (с фильтрацией заглушек)
    const hasMultiplePlatforms = (interview, qIdx) => {
      ...
      // Обе платформы имеют реальные ответы (не заглушки) и содержание различается
    ...
    return () => h('div', { class: 'interview-display' }, [
      ...
      props.result.interviews.length > 0 && h('div', { class: 'interview-detail' }, [
        ...
        // Selection Reason - Причина выбора
        ...
        // Q&A Conversation Thread - формат вопрос-ответ
              ...
              answerText && h('div', { class: ['qa-answer', { 'answer-placeholder': isPlaceholder }] }, [
                ...
                h('div', { class: 'qa-content' }, [
                  h('div', { class: 'qa-answer-header' }, [
                    ...
                    // Кнопка переключения платформ (показывать только при наличии реальных ответов с обеих платформ)
                  ...
                  // Expand/Collapse Button (не показывать для текста-заглушки)
      ...
      h('div', { class: ['quicksearch-content', { 'no-tabs': !showTabs.value }] }, [
        ...
        activeTab.value === 'edges' && hasEdges.value && h('div', { class: 'edges-panel' }, [
          h('div', { class: 'panel-header' }, [
            ...
            h('span', { class: 'panel-count' }, `Всего ${props.result.edges.length} зап.`)
          ]),
...
const activeStep = computed(() => {
  ...
  // Если нет active, вернуть последний done шаг
...
const workflowSteps = computed(() => {
  ...
  steps.push({
    ...
    meta: completeStatus === 'active' ? 'ЗАВЕРШЕНИЕ' : ''
  })
...
const renderMarkdown = (content) => {
  ...
  // Убрать заголовок второго уровня (## xxx) в начале, т.к. он уже отображён снаружи
  let processedContent = content.replace(/^##\s+.+\n+/, '')

  // Обработать блоки кода
  let html = processedContent.replace(/```(\w*)\n([\s\S]*?)```/g, '<pre class="code-block"><code>$2</code></pre>')

  // Обработать инлайн-код
  ...
  // Обработать блоки цитат
  html = html.replace(/^> (.+)$/gm, '<blockquote class="md-quote">$1</blockquote>')
  
  // Обработать списки - поддержка вложенных списков
  ...
  // Обернуть ненумерованные списки
  html = html.replace(/(<li class="md-li"[^>]*>.*?<\/li>\s*)+/g, '<ul class="md-ul">$&</ul>')
  // Обернуть нумерованные списки
  html = html.replace(/(<li class="md-oli"[^>]*>.*?<\/li>\s*)+/g, '<ol class="md-ol">$&</ol>')

  // Очистить все пробелы между элементами списка
  html = html.replace(/<\/li>\s+<li/g, '</li><li')
  // Очистить пробелы после открывающего тега списка
  html = html.replace(/<ul class="md-ul">\s+/g, '<ul class="md-ul">')
  html = html.replace(/<ol class="md-ol">\s+/g, '<ol class="md-ol">')
  // Очистить пробелы перед закрывающим тегом списка
  html = html.replace(/\s+<\/ul>/g, '</ul>')
  html = html.replace(/\s+<\/ol>/g, '</ol>')
  
  // Обработать жирный и курсив
  html = html.replace(/\*\*(.+?)\*\*/g, '<strong>$1</strong>')
  html = html.replace(/\*(.+?)\*/g, '<em>$1</em>')
  html = html.replace(/_(.+?)_/g, '<em>$1</em>')
  
  // Обработать разделительные линии
  html = html.replace(/^---$/gm, '<hr class="md-hr">')
  
  // Обработать переносы - пустые строки становятся разделителями абзацев, одиночные переносы -> <br>
  html = html.replace(/\n\n/g, '</p><p class="md-p">')
  html = html.replace(/\n/g, '<br>')
  ...
  // Очистить пустые абзацы
  html = html.replace(/<p class="md-p"><\/p>/g, '')
  html = html.replace(/<p class="md-p">(<h[2-5])/g, '$1')
  html = html.replace(/(<\/h[2-5]>)<\/p>/g, '$1')
  html = html.replace(/<p class="md-p">(<ul|<ol|<blockquote|<pre|<hr)/g, '$1')
  html = html.replace(/(<\/ul>|<\/ol>|<\/blockquote>|<\/pre>)<\/p>/g, '$1')
  // Очистить теги <br> вокруг блочных элементов
  html = html.replace(/<br>\s*(<ul|<ol|<blockquote)/g, '$1')
  html = html.replace(/(<\/ul>|<\/ol>|<\/blockquote>)\s*<br>/g, '$1')
  // Очистить <p><br> перед блочными элементами (из-за лишних пустых строк)
  html = html.replace(/<p class="md-p">(<br>\s*)+(<ul|<ol|<blockquote|<pre|<hr)/g, '$2')
  // Очистить последовательные теги <br>
  html = html.replace(/(<br>\s*){2,}/g, '<br>')
  // Очистить <br> между блочными элементами и началом абзаца
  html = html.replace(/(<\/ol>|<\/ul>|<\/blockquote>)<br>(<p|<div)/g, '$1$2')

  // Исправить нумерацию непоследовательных нумерованных списков: когда одноэлементные <ol> разделены абзацами, сохранить возрастающую нумерацию
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      if (newLogs.length > 0) {
        newLogs.forEach(log => {
          ...
          if (log.action === 'section_complete') {
            if (log.details?.content) {
              ...
              // Автоматически развернуть только что сгенерированный раздел
          ...
          if (log.action === 'report_complete') {
            ...
            currentSectionIndex.value = null  // Убедиться, что состояние загрузки очищено
        ...
        nextTick(() => {
          if (rightPanel.value) {
            // Если задача завершена, прокрутить вверх; иначе прокрутить вниз за последними логами
            if (isComplete.value) {
...
// Извлечь финальный ответ - извлечь содержимое раздела из LLM response
const extractFinalContent = (response) => {
  if (!response) return null
  
  // Попытка извлечь содержимое из тега <final_answer>
  ...
  // Попытка найти содержимое после Final Answer: (поддержка разных форматов)
  ...
  // Попытка найти содержимое после Итоговый ответ: / 最终答案:
  const chineseFinalMatch = response.match(/(?:Итоговый ответ|最终答案)[:：]\s*\n*([\s\S]*)$/i)
  if (chineseFinalMatch) {
    return chineseFinalMatch[1].trim()
  }
  
  // Если начинается с ## или # или >, возможно это прямое markdown-содержимое
  const trimmedResponse = response.trim()
  if (trimmedResponse.match(/^[#>]/)) {
    return trimmedResponse
  }
  
  // Если содержимое длинное и содержит markdown-форматирование, попытка удалить процесс размышления
  if (response.length > 300 && (response.includes('**') || response.includes('>'))) {
    // Удалить процесс размышления, начинающийся с Thought:
...
.section-number {
  ...
  font-size: 16px;
  color: #9CA3AF; /* Тёмно-серый, не меняется от состояния */
  font-weight: 500;
}
...
/* Console Logs - соответствует Step3Simulation.vue */
.console-logs {
  background: #000;
  color: #DDD;
  padding: 16px;
