...
class OasisProfileGenerator:
    ...
    def _get_system_prompt(self, is_individual: bool) -> str:
        ...
        base_prompt = "Вы — эксперт по созданию портретов пользователей социальных сетей. Генерируйте детальные, реалистичные персонажи для симуляции общественного мнения, максимально приближенно к реальности. Возвращайте валидный JSON, все строковые значения не должны содержать неэкранированных переносов строк. ОБЯЗАТЕЛЬНО используйте только русский язык во всех полях (bio, persona и т.д.). В конец поля persona ОБЯЗАТЕЛЬНО добавьте: 'Всегда пишет только на русском языке.'"
        return base_prompt
