"""
Генератор профилей OASIS Agent
Преобразует сущности из графа Zep в формат профилей Agent, необходимый для платформы симуляции OASIS

Улучшения и оптимизации:
1. Вызов функции поиска Zep для вторичного обогащения информации о узлах
2. Оптимизированные промпты для генерации очень детальных персонажей
3. Разграничение индивидуальных сущностей и абстрактных групповых сущностей
"""
...
class OasisAgentProfile:
    """Структура данных профиля OASIS Agent"""
    # Общие поля
    user_id: int
    user_name: str
    name: str
    ...
    def to_reddit_format(self) -> Dict[str, Any]:
        ...
        # Добавление дополнительной информации о персонаже (при наличии)
        if self.age:
...
class OasisProfileGenerator:
    ...
    Преобразует сущности из графа Zep в профили Agent, необходимые для симуляции OASIS

    Особенности оптимизации:
    1. Вызов функции поиска по графу Zep для получения более богатого контекста
    2. Генерация очень детальных персонажей (включая базовую информацию, профессиональный опыт, черты характера, поведение в социальных сетях и т.д.)
    3. Разграничение индивидуальных сущностей и абстрактных групповых сущностей
    """
    ...
    # Типы индивидуальных сущностей (требуют генерации конкретного персонажа)
    INDIVIDUAL_ENTITY_TYPES = [
        "student", "alumni", "professor", "person", "publicfigure",
        "expert", "faculty", "official", "journalist", "activist"
    ]

    # Типы групповых/институциональных сущностей (требуют генерации репрезентативного персонажа)
    GROUP_ENTITY_TYPES = [
    ...
    def __init__(
        ...
        # Клиент Zep для поиска богатого контекста
        ...
        if self.zep_api_key:
            try:
                ...
                logger.warning(f"Ошибка инициализации клиента Zep: {e}")
    ...
    def generate_profile_from_entity(
        ...
        """
        Генерация профиля OASIS Agent из сущности Zep
    ...
    def _search_zep_for_entity(self, entity: EntityNode) -> Dict[str, Any]:
        """
        Использование функции гибридного поиска по графу Zep для получения богатой информации о сущности

        У Zep нет встроенного интерфейса гибридного поиска, поэтому необходимо отдельно
        искать рёбра и узлы, а затем объединять результаты.
        ...
        Returns:
            Словарь, содержащий facts, node_summaries, context
        """
        ...
        try:
            # Параллельное выполнение поиска рёбер и узлов
    ...
    def _build_entity_context(self, entity: EntityNode) -> str:
        ...
        Включает:
        1. Информацию о рёбрах (фактах) самой сущности
        2. Детальную информацию о связанных узлах
        3. Богатую информацию, полученную через гибридный поиск Zep
        """
        context_parts = []
        ...
        # 2. Добавление информации о связанных рёбрах (фактах/связях)
        existing_facts = set()
        if entity.related_edges:
            ...
            if relationships:
                context_parts.append("### Связанные факты и связи\n" + "\n".join(relationships))
        ...
        # 4. Использование гибридного поиска Zep для получения более богатой информации
    ...
    def _is_individual_entity(self, entity_type: str) -> bool:
        """Проверка, является ли сущность индивидуальным типом"""
        return entity_type.lower() in self.INDIVIDUAL_ENTITY_TYPES

    def _is_group_entity(self, entity_type: str) -> bool:
        """Проверка, является ли сущность групповым/институциональным типом"""
    ...
    def _generate_profile_with_llm(
        ...
        - Индивидуальная сущность: генерация конкретного персонажа
        - Групповая/институциональная сущность: генерация репрезентативного аккаунта
        """
        ...
        # Попытка генерации с повторными попытками до успеха или достижения максимального числа попыток
        max_attempts = 3
        last_error = None
        ...
        for attempt in range(max_attempts):
            try:
                ...
                # Проверка на усечение (finish_reason не 'stop')
                finish_reason = response.choices[0].finish_reason
                if finish_reason == 'length':
                    logger.warning(f"Вывод LLM усечён (попытка {attempt+1}), попытка восстановления...")
                ...
                try:
                    ...
                    logger.warning(f"Ошибка разбора JSON (попытка {attempt+1}): {str(je)[:80]}")
    ...
    def _fix_truncated_json(self, content: str) -> str:
        """Восстановление усечённого JSON (вывод обрезан ограничением max_tokens)"""
        import re

        # Попытка закрыть усечённый JSON
        ...
        # Простая проверка: если после последней кавычки нет запятой или закрывающей скобки, строка может быть усечена
    ...
    def _try_fix_json(self, content: str, entity_name: str, entity_type: str, entity_summary: str = "") -> Dict[str, Any]:
        """Попытка восстановления повреждённого JSON"""
        import re

        # 1. Сначала попытка восстановления усечённого содержимого
        content = self._fix_truncated_json(content)
        ...
        if json_match:
            ...
            # Поиск всех строковых значений и замена переносов строк в них
            def fix_string_newlines(match):
                s = match.group(0)
                # Замена фактических переносов строк внутри строки на пробелы
                s = s.replace('\n', ' ').replace('\r', ' ')
                # Замена лишних пробелов
                s = re.sub(r'\s+', ' ', s)
                return s

            # Сопоставление строковых значений JSON
            ...
            try:
                ...
                # 5. Если всё ещё не удаётся, попытка более агрессивного восстановления
                try:
                    # Удаление всех управляющих символов
                    json_str = re.sub(r'[\x00-\x1f\x7f-\x9f]', ' ', json_str)
                    # Замена последовательных пробелов
        ...
        persona_match = re.search(r'"persona"\s*:\s*"([^"]*)', content)  # Может быть усечено

        bio = bio_match.group(1) if bio_match else (entity_summary[:200] if entity_summary else f"{entity_type}: {entity_name}")
        persona = persona_match.group(1) if persona_match else (entity_summary or f"{entity_name} — это {entity_type}.")

        # Если удалось извлечь значимое содержимое, помечаем как восстановленное
...
Сгенерируйте JSON со следующими полями:

1. bio: Краткое описание для социальной сети, 200 символов
2. persona: Детальное описание персонажа (2000 символов сплошного текста), должно включать:
   - Базовую информацию (возраст, профессия, образование, местоположение)
   - Предысторию (важные события, связь с событием, социальные связи)
   - Черты характера (тип MBTI, основные черты характера, способ выражения эмоций)
   - Поведение в социальных сетях (частота публикаций, предпочтения по контенту, стиль взаимодействия, языковые особенности)
   - Позиция и взгляды (отношение к теме, контент, который может вызвать гнев/сочувствие)
   - Уникальные черты (любимые выражения, особый опыт, личные увлечения)
   - Личная память (важная часть персонажа, нужно описать связь индивида с событием, а также его действия и реакции в рамках события)
...
8. interested_topics: Массив интересных тем
...
- Содержимое должно соответствовать информации о сущности
...
1. bio: Описание официального аккаунта, 200 символов, профессиональное и корректное
2. persona: Детальное описание настроек аккаунта (2000 символов сплошного текста), должно включать:
   - Базовую информацию об организации (официальное название, тип организации, история создания, основные функции)
   - Позиционирование аккаунта (тип аккаунта, целевая аудитория, ключевые функции)
   ...
   - Позиция и отношение (официальная позиция по ключевым темам, способ работы со спорными вопросами)
   - Особые указания (портрет представляемой группы, привычки ведения аккаунта)
   - Институциональная память (важная часть персонажа организации, нужно описать связь организации с событием, а также её действия и реакции в рамках события)
3. age: Фиксированное значение 30 (виртуальный возраст институционального аккаунта)
4. gender: Фиксированное значение "other" (для институциональных аккаунтов используется other для обозначения неличного характера)
5. mbti: Тип MBTI для описания стиля аккаунта, например ISTJ — строгий и консервативный
6. country: Страна (на русском, например "Россия")
...
- Высказывания институционального аккаунта должны соответствовать его позиционированию"""
    ...
    def _generate_profile_rule_based(
        ...
        # Генерация различных персонажей в зависимости от типа сущности
    ...
    def _normalize_entity_names(self, entities: List[EntityNode]) -> None:
        ...
        Изменяет entity.name in-place для каждой сущности.
        Использует один вызов LLM для пакетной нормализации.
        """
        if not entities:
            return
...
Если имя — это страна или географический регион (например, «Исландия», «Великобритания», «Япония»), верни его без изменений.
...
Верни ТОЛЬКО JSON-массив с нормализованными именами в том же порядке и того же размера. Никакого другого текста."""
        ...
        try:
            ...
            if isinstance(normalized, list) and len(normalized) == len(entities):
                ...
                logger.warning(f"LLM вернул массив неверного размера ({len(normalized) if isinstance(normalized, list) else 'не массив'} вместо {len(entities)}), пропуск нормализации")

        except Exception as e:
            logger.warning(f"Ошибка нормализации имён сущностей: {e}, продолжаем без нормализации")

    def set_graph_id(self, graph_id: str):
        """Установка ID графа для поиска Zep"""
        self.graph_id = graph_id
    ...
    def generate_profiles_from_entities(
        ...
        """
        Пакетная генерация профилей Agent из сущностей (с поддержкой параллельной генерации)

        Args:
            entities: Список сущностей
            use_llm: Использовать ли LLM для генерации детальных персонажей
            progress_callback: Функция обратного вызова прогресса (current, total, message)
            graph_id: ID графа, используется для поиска Zep с целью получения более богатого контекста
            parallel_count: Количество параллельных потоков, по умолчанию 5
            realtime_output_path: Путь файла для записи в реальном времени (если указан, запись происходит после генерации каждого профиля)
            output_platform: Формат выходной платформы ("reddit" или "twitter")

        Returns:
            Список профилей Agent
        """
        import concurrent.futures
        ...
        # Нормализация имён сущностей (именительный падеж)
        self._normalize_entity_names(entities)

        total = len(entities)
        profiles = [None] * total  # Предварительное выделение списка для сохранения порядка
        completed_count = [0]  # Используется список для возможности изменения в замыкании
        lock = Lock()
        ...
        def save_profiles_realtime():
            ...
            with lock:
                ...
                try:
                    ...
                    logger.warning(f"Ошибка сохранения профилей в реальном времени: {e}")

        def generate_single_profile(idx: int, entity: EntityNode) -> tuple:
            """Рабочая функция для генерации одного профиля"""
            entity_type = entity.get_entity_type() or "Entity"
            ...
            try:
                ...
                logger.error(f"Ошибка генерации персонажа для сущности {entity.name}: {str(e)}")
        ...
        print(f"\n{'='*60}")
        print(f"Генерация персонажей завершена! Всего сгенерировано {len([p for p in profiles if p])} агентов")
        print(f"{'='*60}\n")

        return profiles

    def _print_generated_profile(self, entity_name: str, entity_type: str, profile: OasisAgentProfile):
        """Вывод сгенерированного персонажа в консоль в реальном времени (полное содержимое, без усечения)"""
        separator = "-" * 70

        # Построение полного содержимого вывода (без усечения)
        ...
        output_lines = [
            ...
            f"Интересные темы: {topics_str}",
            separator
        ]

        output = "\n".join(output_lines)

        # Вывод только в консоль (для избежания дублирования, логгер не выводит полное содержимое)
        print(output)
    ...
    def save_profiles(
        ...
        """
        Сохранение профилей в файл (выбор правильного формата в зависимости от платформы)

        Требования к форматам платформы OASIS:
        - Twitter: формат CSV
        - Reddit: формат JSON
    ...
    def _save_twitter_csv(self, profiles: List[OasisAgentProfile], file_path: str):
        """
        Сохранение профилей Twitter в формате CSV (в соответствии с официальными требованиями OASIS)

        Поля CSV, требуемые OASIS Twitter:
        - user_id: ID пользователя (начиная с 0 по порядку в CSV)
        - name: Настоящее имя пользователя
        - username: Имя пользователя в системе
        - user_char: Детальное описание персонажа (внедряется в системный промпт LLM для управления поведением Agent)
        - description: Краткое публичное описание (отображается на странице профиля)

        Различие между user_char и description:
        - user_char: Для внутреннего использования, системный промпт LLM, определяет как Agent думает и действует
        - description: Для внешнего отображения, краткое описание, видимое другим пользователям
        """
        import csv

        # Убеждаемся, что расширение файла — .csv
        ...
        with open(file_path, 'w', newline='', encoding='utf-8') as f:
            ...
            for idx, profile in enumerate(profiles):
                ...
                row = [
                    idx,                    # user_id: порядковый ID, начиная с 0
                    profile.name,           # name: настоящее имя
                    profile.user_name,      # username: имя пользователя
                    user_char,              # user_char: полный персонаж (для внутреннего использования LLM)
                    description             # description: краткое описание (для внешнего отображения)
                ]
                writer.writerow(row)

        logger.info(f"Сохранено {len(profiles)} профилей Twitter в {file_path} (формат OASIS CSV)")

    def _normalize_gender(self, gender: Optional[str]) -> str:
        """
        Нормализация поля gender в формат, требуемый OASIS (на английском)
        ...
        # Маппинг на русском и китайском
        gender_map = {
            "мужской": "male",
            "женский": "female",
            "организация": "other",
    ...
    def _save_reddit_json(self, profiles: List[OasisAgentProfile], file_path: str):
        """
        Сохранение профилей Reddit в формате JSON

        Используется формат, согласованный с to_reddit_format(), для корректного чтения OASIS.
        Обязательно должно содержать поле user_id — это ключ для сопоставления в agent_graph.get_agent() OASIS!

        Обязательные поля:
        - user_id: ID пользователя (целое число, используется для сопоставления с poster_agent_id в initial_posts)
        ...
        data = []
        for idx, profile in enumerate(profiles):
            # Использование формата, согласованного с to_reddit_format()
            item = {
                "user_id": profile.user_id if profile.user_id is not None else idx,  # Ключевое: обязательно должен содержать user_id
                ...
                # Обязательные поля OASIS — убеждаемся, что у всех есть значения по умолчанию
        ...
        logger.info(f"Сохранено {len(profiles)} профилей Reddit в {file_path} (формат JSON, включает поле user_id)")
