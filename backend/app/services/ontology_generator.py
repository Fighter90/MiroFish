...
ONTOLOGY_SYSTEM_PROMPT = """Вы — профессиональный эксперт по проектированию онтологий графов знаний. Ваша задача — проанализировать исходный текст и требования к моделированию, разработать типы сущностей и типы связей, подходящие для **моделирования общественного мнения в социальных сетях**.
...
- Страны или географические регионы как отдельные агенты (Исландия, Великобритания, Португалия, Бельгия, Россия, Япония и т.д. — ЗАПРЕЩЕНЫ). Допустимы только конкретные организации и лица из этих стран
- Отрасли или секторы экономики как отдельные агенты (IT-сектор, сельское хозяйство, промышленность, энергетика, металлургия, химическая промышленность — ЗАПРЕЩЕНЫ). Допустимы только конкретные организации, компании или социальные группы работников этих отраслей

**Критически важно для русского языка**:
- Все имена сущностей и примеры ОБЯЗАТЕЛЬНО должны быть в **именительном падеже** (кто? что?)
- ПРАВИЛЬНО: «Промышленные рабочие», «Депутаты Государственной Думы», «IT-специалисты», «Молодёжь», «Пенсионеры»
- НЕПРАВИЛЬНО: «промышленными работниками» (творительный), «государственных служащих» (родительный), «офисных сотрудников» (родительный)
- Имена должны звучать как названия аккаунтов в социальных сетях

**Язык вывода**:
- Весь вывод (описания, имена, примеры) ОБЯЗАТЕЛЬНО на русском языке
- Не смешивайте русский и английский языки в описаниях

## Формат вывода

Выведите данные в формате JSON со следующей структурой:
...
- Person: Любое физическое лицо (используется, когда не подходит ни один из перечисленных выше конкретных типов)
...
- Organization: Любая организация/учреждение (используется, когда не подходит ни один из перечисленных выше конкретных типов)
...
"""
        ...
        message += """
На основе приведённого выше содержимого разработайте типы сущностей и типы связей, подходящие для моделирования общественного мнения.
...
6. Все примеры сущностей (examples) ОБЯЗАТЕЛЬНО в именительном падеже (кто? что?). Не «государственных служащих», а «Государственные служащие». Не «производственными работниками», а «Промышленные рабочие».
7. Не включайте страны (Исландия, Великобритания, Япония) как примеры сущностей — только организации и людей
"""
