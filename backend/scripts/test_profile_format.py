...
def test_profile_formats():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        print(f"   Файл: {twitter_path}")
        print(f"   Количество строк: {len(rows)}")
        print(f"   Заголовки: {list(rows[0].keys())}")
        ...
        print(f"   Файл: {reddit_path}")
        print(f"   Количество записей: {len(reddit_data)}")
        print(f"   Поля: {list(reddit_data[0].keys())}")
        print(f"\n   Пример данных (запись 1):")
        print(json.dumps(reddit_data[0], ensure_ascii=False, indent=4))

        # Проверка полей детального формата
...
def show_expected_formats():
    """Отображение ожидаемых форматов OASIS"""
    print("\n" + "=" * 60)
    print("Справочник ожидаемых форматов OASIS Profile")
    print("=" * 60)
